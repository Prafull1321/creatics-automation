...
describe("CineJoy Testcases.", function() {
    ...
    const Production_URL = "https://creatics.org/";
      ...
      it("Validate Post production main page details added by the admin user" , () =>{
        cy.contains("Reality Show: The Unscripted Star ");
        cy.contains("Contests");
        cy.contains("Editor's Room");
        cy.contains("Polls");
        cy.contains("Editing Sneak Peeks");
        cy.contains("Producer Updates");
        cy.contains("Ask the Team Anything");
      });
      ...
      it("Validate Polls, sneak peeks, updates and ask team anything functionality under post production" , () =>{
        hitmakerPage.verifyPostprodPollsSneakpeekAndUpdate();
        hitmakerPage.verifyAskTeamAnythingPostprod();
      });
